...
const create = async(req, res) => {
    try{
        ...
        res.status(201).json({ message: 'Tâche ajoutée', taskId: result.insertedId });
...
const update_by_id = async (req, res) => {
    try {
        ...
        if (result.modifiedCount === 0) {
            return res.status(404).json({ message: 'Tâche non trouvée ou aucune modification effectuée' });
        }
...
const delet_by_name = async (req, res) => {
    try {
        ...
        if (!nom) {
            return res.status(400).json({ message: "Le nom est requis pour la suppression." });
        }
...
const get_element_by_status = async(req, res) => {
    try {
        ...
        const task_terminer = await db.collection('tasks').countDocuments({ status: "terminée" });

        // Compter le nombre de tâches non terminées
        const task_en_cour = await db.collection('tasks').countDocuments({ status: "non terminer" });
...
const filtrage_par_status = async (req, res) => {
    try {
        const db = getDB();
        const tasks = await db.collection('tasks').find().sort({ status: 1 }).toArray(); // Ajout de .toArray() ici
...
const triage_par_nom = async (req, res) => {
    try{
        const db = getDB();
        const tasks = await db.collection('tasks').find().sort({ nom: 1 }).toArray();
...
export default { read, create, update_by_id, delet_by_id, update_by_name, delet_by_name, get_element_by_status, filtrage_par_status, triage_par_nom};
